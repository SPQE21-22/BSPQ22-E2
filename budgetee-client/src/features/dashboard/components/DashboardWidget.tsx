...
export const DashboardWidget = ({ title, to, linkText = 'See all', children }: DashboardWidgetProps) => {
  return (
    <div className='relative bg-white shadow shadow-slate-400 col-span-1 rounded-md w-full overflow-hidden h-96 flex flex-col items-stretch'>
      <h2 className='py-1.5 px-3 font-medium text-lg'>{title}</h2>
      <div className={`px-2 h-full ${to && 'pb-16'}`}>
        {children}
      </div>
      {to && (
